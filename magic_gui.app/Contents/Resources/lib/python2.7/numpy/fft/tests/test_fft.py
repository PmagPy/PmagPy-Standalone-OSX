...
import numpy as np
try:
    use_fastnumpy = np.use_fastnumpy
...
class TestFFT1D(TestCase):
    ...
    def test_2d_array(self):
        ...
        b = np.array([np.fft.fft(a[i]) for i in xrange(m)])
    ...
    def test_nd_shape(self):
        for nd in xrange(1, 9):
            ...
            for dim in xrange(1, nd + 1):
                b = a.copy()
                axes = random.sample(range(nd), dim)
                shape = [random.randint(2, 5) for dum in xrange(dim)]
                for i in xrange(dim):
